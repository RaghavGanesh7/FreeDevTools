---
title: "Pacman Files - Manage Files with Pacman | Free DevTools"
name: pacman-f
path: /freedevtools/tldr/linux/pacman-f
canonical: "https://hexmos.com/freedevtools/tldr/linux/pacman-f/"
description: "Manage files efficiently with Pacman. View file information and lists of files installed by packages. Free online tool, no registration required."
category: linux
keywords:
  - pacman files management
  - pacman file listing
  - linux package file management
  - pacman file information
  - arch linux file management
  - pacman installed files
  - linux file utilities
  - pacman package files
  - system file management pacman
  - pacman command line files
features:
  - Lists all files installed by packages.
  - Displays file paths and sizes.
  - Identifies package ownership of files.
  - Allows quick checking of installed files.
  - Facilitates troubleshooting file-related issues.
ogImage: "https://hexmos.com/freedevtools/t/tool-banners/json-utilities-banner.png"
twitterImage: "https://hexmos.com/freedevtools/t/tool-banners/json-utilities-banner.png"
---

# pacman -F

> This command is an alias of `pacman --files`.

- View documentation for the original command:

`tldr pacman files`
