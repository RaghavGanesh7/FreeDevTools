...
const convertCurlToFetch = (curlCommand: string, options: ConversionOptions = {}): string => {
  try {
    const { includeComments = true, useAsync = true, includeErrorHandling = true } = options;
    
    // Parse the cURL command
    const parsedCurl = parseCurlCommand(curlCommand);
    
    // Generate JavaScript fetch code
    let fetchCode = generateFetchCode(parsedCurl, { includeComments, useAsync, includeErrorHandling });
    
    return fetchCode;
  } catch (error) {
    throw new Error(`Failed to convert cURL command: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
};
...
const parseCurlCommand = (curlCommand: string): ParsedCurl => {
  ...
  const urlMatch = command.match(/(?:^|\s)(?:-u|--url)\s+([^\s]+)/) || 
  ...
  // Parse data
  const dataMatch = command.match(/(?:^|\s)(?:-d|--data|--data-raw)\s+(['"])(.*?)\1/);
  if (dataMatch) {
...
const generateFetchCode = (parsed: ParsedCurl, options: ConversionOptions): string => {
  ...
  // Options object
  const hasOptions = parsed.method !== 'GET' || Object.keys(parsed.headers).length > 0 || parsed.data || parsed.auth;
...
const CurlToJsFetch: React.FC = () => {
  ...
  const handleConversion = useCallback(() => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : "Failed to convert cURL command";
            ...
            <Card className="tool-card-bg">
              ...
              <CardContent className="space-y-6">
                ...
                <div className="space-y-4">
                  <h4 className="font-medium text-slate-900 dark:text-slate-100">Conversion Options</h4>
                ...
                {input && output && (
                  <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-md">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                      <div>
                        <span className="text-slate-600 dark:text-slate-400">Input Length:</span>
                        <span className="ml-2 font-mono text-blue-800 dark:text-blue-200">
                          {input.length} chars
                        </span>
                      </div>
                      <div>
                        <span className="text-slate-600 dark:text-slate-400">Output Length:</span>
                        <span className="ml-2 font-mono text-blue-800 dark:text-blue-200">
                          {output.length} chars
                        </span>
                      </div>
                      <div>
                        <span className="text-slate-600 dark:text-slate-400">Lines Generated:</span>
                        <span className="ml-2 font-mono text-blue-800 dark:text-blue-200">
                          {output.split('\n').length} lines
                        </span>
                      </div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-slate-800 dark:text-slate-400 space-y-4">
                  <p>
                    Converting cURL commands to JavaScript fetch API calls is essential for modern web development. This tool helps you transform command-line HTTP requests into browser-compatible JavaScript code that can be used in web applications.
                  </p>
                    ...
                    <div className="space-y-3">
                      ...
                      <ul className="space-y-2 text-sm">
                        <li className="flex items-start">
                          ...
                          <span>
                            <strong>-X METHOD:</strong> HTTP method (GET, POST, PUT, DELETE)
                          </span>
                        </li>
                        <li className="flex items-start">
                  ...
                  <div className="bg-slate-100 dark:bg-slate-700 rounded p-4 font-mono text-sm">
                    ...
                    <div className="mb-4">
                      <div className="text-blue-600 dark:text-blue-400 mb-1">cURL:</div>
                      <div className="text-slate-800 dark:text-slate-200 break-all">
                        curl -X POST -H "Content-Type: application/json" -d '{"key"+':'+"value"}' https://api.example.com
                      </div>
                    </div>
                    <div>
                      <div className="text-green-600 dark:text-green-400 mb-1">JavaScript Fetch:</div>
  ...
  body: '{"key":"value"}'
            ...
            <Card>
              <CardHeader>
                <CardTitle>Advanced Conversion Features and Best Practices</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="text-slate-800 dark:text-slate-400 space-y-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <ul className="space-y-3 ml-11">
                        <li className="flex items-start">
                          <span className="w-2 h-2 bg-blue-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                          <div>
                            <strong>Authentication:</strong> Converts -u flag to Authorization headers
                          </div>
                        </li>
                        <li className="flex items-start">
                          <span className="w-2 h-2 bg-blue-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                          <div>
                            <strong>Headers:</strong> Maps all -H flags to fetch headers object
                          </div>
                        </li>
                        <li className="flex items-start">
                          <span className="w-2 h-2 bg-blue-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                          <div>
                            <strong>Request Body:</strong> Handles -d, --data, and --data-raw flags
                          </div>
                        </li>
                        <li className="flex items-start">
                          <span className="w-2 h-2 bg-blue-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                          <div>
                            <strong>Method Detection:</strong> Auto-detects POST for data requests
                          </div>
                        </li>
                      </ul>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <ul className="space-y-3 ml-11">
                        <li className="flex items-start">
                          <span className="w-2 h-2 bg-green-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                          <div>
                            <strong>Error Handling:</strong> Always check response.ok before parsing
                          </div>
                        </li>
                        <li className="flex items-start">
                          <span className="w-2 h-2 bg-green-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                          <div>
                            <strong>Async/Await:</strong> Use modern syntax for better readability
                          </div>
                        </li>
                        <li className="flex items-start">
                          <span className="w-2 h-2 bg-green-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                          <div>
                            <strong>Content-Type:</strong> Set appropriate headers for data requests
                          </div>
                        </li>
                        <li className="flex items-start">
                          <span className="w-2 h-2 bg-green-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                          <div>
                            <strong>CORS:</strong> Consider cross-origin policies in browsers
                          </div>
                        </li>
                      </ul>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <ul className="space-y-3 ml-11">
                        <li className="flex items-start">
                          <span className="w-2 h-2 bg-purple-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                          <div>
                            <strong>API Documentation:</strong> Convert example cURL commands
                          </div>
                        </li>
                        <li className="flex items-start">
                          <span className="w-2 h-2 bg-purple-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                          <div>
                            <strong>Frontend Development:</strong> Implement API calls in web apps
                          </div>
                        </li>
                        <li className="flex items-start">
                          <span className="w-2 h-2 bg-purple-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                          <div>
                            <strong>Testing:</strong> Create test scripts from cURL commands
                          </div>
                        </li>
                        <li className="flex items-start">
                          <span className="w-2 h-2 bg-purple-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                          <div>
                            <strong>Migration:</strong> Update legacy XMLHttpRequest code
                          </div>
                        </li>
                      </ul>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <ul className="space-y-3 ml-11">
                        <li className="flex items-start">
                          <span className="w-2 h-2 bg-orange-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                          <div>
                            <strong>Modern Browsers:</strong> Fetch API supported in all current browsers
                          </div>
                        </li>
                        <li className="flex items-start">
                          <span className="w-2 h-2 bg-orange-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                          <div>
                            <strong>Polyfills:</strong> Use fetch polyfill for older browser support
                          </div>
                        </li>
                        <li className="flex items-start">
                          <span className="w-2 h-2 bg-orange-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                          <div>
                            <strong>Node.js:</strong> Use node-fetch package for server-side code
                          </div>
                        </li>
                        <li className="flex items-start">
                          <span className="w-2 h-2 bg-orange-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                          <div>
                            <strong>React Native:</strong> Native fetch support included
                          </div>
                        </li>
                      </ul>
                    </div>
                  ...
                  <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/10 dark:to-indigo-900/10 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
                    <p className="text-sm text-blue-800 dark:text-blue-200">
                      <strong>💡 Pro Tip:</strong> When converting cURL commands for production use, always add proper error handling and consider implementing retry logic for network failures. The generated code provides a solid foundation, but you may need to customize it based on your specific requirements and error handling strategies.
                    </p>
                  </div>
                </div>
              </CardContent>
