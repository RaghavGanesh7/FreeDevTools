...
/**
 * Generate paginated paths for SVG icons categories
 */
export async function generateSvgIconsStaticPaths() {
...
export async function createCategoryIconsMap() {
  ...
  for (const [key, clusterData] of Object.entries(iconsData.clusters)) {
    const icons = clusterData.fileNames.map((fileObj: any) => {
      ...
      return {
        name: iconName,
        url: `/freedevtools/svg_icons/${clusterData.name}/${iconName}/`,
        description:
